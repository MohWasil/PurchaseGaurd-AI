# """
# Streamlit Frontend - PurchaseGuard AI Phase 2
# Receipt upload, processing, and dashboard
# """
...
#     with col1:
#         st.markdown("- 📸 Receipt scanning & OCR")
#         st.markdown("- 🤖 AI data extraction (Hugging Face)")
#         st.markdown("- ⏰ Return deadline tracking")
#     with col2:
#         st.markdown("- 🛡️ Warranty expiry alerts")
#         st.markdown("- 💰 Price drop monitoring (coming)")
...
#         st.markdown("- 🔔 View Alerts")
    
#     # Main page
#     st.title("🛡️ PurchaseGuard Dashboard")
...
#         if p.get("return_deadline") and 
#         datetime.fromisoformat(p["return_deadline"].replace("Z", "+00:00").replace("+00:00", "")) > datetime.utcnow()
#     ) if purchases else 0
...
#     with col3:
#         st.metric("Active Warranties", total_purchases)
#     with col4:
#         st.metric("Upcoming Returns", upcoming_returns)
...
#                 ):
#                     st.write(f"**Date:** {purchase.get('purchase_date', 'N/A')[:10]}")
...
#                     if st.button("Delete", key=f"delete_{purchase['id']}"):
...
#                 "medium": "🟡",
#                 "low": "🟢"
#             }.get(alert.get("priority", "low"), "⚪")
            
#             st.markdown(f"{priority_color} **{alert.get('type', 'Unknown')}:** {alert.get('message', '')}")
#     else:
#         st.info("🔔 No alerts. Keep uploading receipts!")
    
#     # Footer
#     st.markdown("---")
#     st.markdown("**⚠️ Phase 2 Complete** - Receipt Upload & AI Processing Working")
#     st.markdown("**Next:** Price tracking, email integration, notifications")
